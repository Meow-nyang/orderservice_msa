...
pipeline {
    ...
    environment {
        ...
        ECR_URL = "780274834139.dkr.ecr.ap-northeast-2.amazonaws.com/orderservice-image"
    ...
    stages {
        ...
        stage('Build Docker Image & Push to AWS ECR') {
            ...
            steps {
                script {
                    // jenkins에 저장된 credential를 사용하여 AWS 자격증명을 설정.
                    withAWS(region: "${REGION}", credentials: "aws-key") {
                        ...
                        changedServices.each { service ->
                            ...
                            # Docker에게 psuh 명령을 내리면 지정된 URL로 push할 수 있게 설정.
                            # 자동으로 로그인 도구를 쓰게 설정
                            mkdir -p ~/.docker
